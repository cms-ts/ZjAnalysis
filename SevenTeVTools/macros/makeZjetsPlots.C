...
makeZjetsPlots (int whichobservable, int whichjet, int whichlepton, bool inclusiveMultiplicity, bool isPDFComparison)
{
  ...
  string s                = "/afs/infn.it/ts/user/marone/html/ZJets/FinalPlotsForAN/v58/FinalPlotsArticle/Ele/";
  if (lepton==2) string s = "/afs/infn.it/ts/user/marone/html/ZJets/FinalPlotsForAN/v58/FinalPlotsArticle/Mu/";
  if (lepton==3) string s = "/afs/infn.it/ts/user/marone/html/ZJets/FinalPlotsForAN/v58/FinalPlotsArticle/Combined/";
  ...
  while ((tobj = iter.Next ()))
	...
	for (Int_t ovo=0;ovo<nRivetPoints;ovo++) {
	  ...
	  leadingRatioPlot2Stat->SetPoint(ovo,dummyXvar,(dummyYvar/dummyNorm)/leadingSystematics->GetBinContent(ovo+1));
	...
	for (Int_t ovo=0;ovo<nRivetPoints;ovo++) {
	  ...
	  leadingRatioPlot2->SetPoint(ovo,dummyXvar,(dummyYvar/dummyNorm)/leadingSystematics->GetBinContent(ovo+1));
	...
	leadingRivetPlot22ENV = (TGraphAsymmErrors *) leadingRivetPlot2->Clone ();
	TGraphAsymmErrors *leadingRatioPlot22ENV;
	leadingRatioPlot22ENV = (TGraphAsymmErrors *) leadingRatioPlot2->Clone ();
	...
	for (Int_t ovo=0;ovo<nRivetPoints;ovo++) {
	  ...
	  //Set Ratio Plot
	  leadingRatioPlot22ENV->SetPoint(ovo,dummyXvar,(dummyYvar/dummyNorm)/leadingSystematics->GetBinContent(ovo+1));
	...
	for (Int_t ovo=0;ovo<nRivetPoints;ovo++) {
	  ...
	  leadingRatioPlot3->SetPoint(ovo,dummyXvar,(dummyYvar/dummyNorm)/leadingSystematics->GetBinContent(ovo+1));
	...
	if (!isPDFComparison){
	  legenddx_d->AddEntry (leadingRivetPlot1Stat, "Plot12 (0,1j@NLO #leq4j@LO +PS)", "PLEF");
	  legenddx_d->AddEntry (leadingRivetPlot2Stat, "Plot2+Pythia6 (Z+1j@NLO +PS)", "PLEF");
	  legenddx_d->AddEntry (leadingRivetPlot3, "Plot3+Pythia6 (#leq4j@LO +PS)", "PLEF");
	}
	else{
	  legenddx_d->AddEntry (leadingRivetPlot1Stat, "Plot12 CT10", "PLEF");
	  legenddx_d->AddEntry (leadingRivetPlot2Stat, "Plot12 NNPDF", "PLEF");
	  legenddx_d->AddEntry (leadingRivetPlot3, "Plot12 MSTW2008", "PLEF");
	}
	...
	leadingRatioPlot1Q->SetFillColor(kBlue-7);
	leadingRatioPlot1Q->SetFillStyle(3004);
	leadingRatioPlot1Q->SetLineColor(kBlue-7);
	leadingRatioPlot1Q->SetMarkerColor(kBlue-7);
	leadingRatioPlot1Q->SetLineWidth(0.6);
	leadingRatioPlot1Q->Draw("5");
	}
        ...
        TLegend *legendPlot1;
        legendPlot1 = new TLegend (0.19, 0.05, 0.40, 0.22);       
        legendPlot1->SetFillColor (0);
        legendPlot1->SetFillStyle (0);
        legendPlot1->SetBorderSize (0);
        legendPlot1->AddEntry (leadingRatioPlot1Qcut, "Theory unc. (gen)", "F");
        legendPlot1->AddEntry (leadingRatioPlot1Stat, "Statistical unc. (gen)", "F");
	...
	leadingRatioPlot22ENV->SetFillColor(kGreen-7);
	leadingRatioPlot22ENV->SetFillStyle(1001);
	leadingRatioPlot22ENV->SetLineColor(kGreen-6);
	leadingRatioPlot22ENV->SetMarkerColor(kGreen-6);
	leadingRatioPlot22ENV->SetLineWidth(0.6);
	leadingRatioPlot22ENV->Draw("5");
	
	leadingRatioPlot2Stat->SetFillColor(kGreen+8);
	...
	TLegend *legendPlot2;
	legendPlot2 = new TLegend (0.19, 0.05, 0.40, 0.22);	   
	...
	legendPlot2->AddEntry (leadingRatioPlot2Stat, "Stat. unc. (gen)", "F");
	...
	if (isPDFComparison){
	  leadingRatioPlot32ENV->SetFillColor(kOrange-6);
	  leadingRatioPlot32ENV->SetFillStyle(1001);
	  leadingRatioPlot32ENV->SetLineColor(kOrange-6);
	  leadingRatioPlot32ENV->SetMarkerColor(kOrange-6);
	  leadingRatioPlot32ENV->SetLineWidth(0.6);
	  leadingRatioPlot32ENV->Draw("5");
	}	
      
	leadingRatioPlot3->SetFillColor(kOrange-2);
	...
	TLegend *legendPlot3;
	if (isPDFComparison) legendPlot3 = new TLegend (0.19, 0.31, 0.39, 0.44);	   
	else legendPlot3 = new TLegend (0.19, 0.33, 0.40, 0.39);	   
	legendPlot3->SetFillColor (0);
	legendPlot3->SetFillStyle (0);
	legendPlot3->SetBorderSize (0);
	if (isPDFComparison) legendPlot3->AddEntry (leadingRatioPlot32ENV, "Theory unc. (gen)", "F");
	legendPlot3->AddEntry (leadingRatioPlot3, "Statistical unc. (gen)", "F");
	legendPlot3->Draw ("same");
  ...
  while ((tobjRivet= iterRivet.Next ()))
      ...
      if (nameRivet == rivet_data || (isPDFComparison && (nameRivet == rivet_data_minus) ) )
	{
	  if (!isPDFComparison || !readTH1F) {
	    ...
	    TGraphAsymmErrors *leadingRivet=new TGraphAsymmErrors(histo);
	  }
	}

      if (etaFolded && use_case==3) {
	double xx1temp, xx2temp, yy1temp, yy2temp;
	for (int mdm=leadingRivet->GetN()/2; mdm<leadingRivetPlot1QUP->GetN(); mdm++) {
	  leadingRivet->GetPoint(mdm,xx1temp,yy1temp);
	  leadingRivet->GetPoint(leadingRivetPlot1QUP->GetN()-mdm-1,xx2temp,yy2temp);
